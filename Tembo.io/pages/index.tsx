...
export default function index() {
  ...
  return (
    <>
      ...
      <NavComponent />
      <Hero />
      {/* Coinbase wallet buttons intergration */}

      <div className="flex items-center justify-center">
      <button onClick={connect} className="bg-green-800 hover:bg-green-300 text-white font-bold py-2 px-4 rounded-full">Connect to Coinbase</button>
      {active ? <span>Connected with Coinbase <b>{account}</b></span> : <span>Not connected</span>}
      <button onClick={disconnect} className="bg-green-800 hover:bg-green-300 text-white font-bold py-2 px-4 rounded-full">Disconnect</button>
    </div>
      <AboutSection />
      <Discover />
      <MarketPlace />
